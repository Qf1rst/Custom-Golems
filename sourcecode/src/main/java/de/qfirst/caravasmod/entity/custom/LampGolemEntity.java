...
public class LampGolemEntity extends TameableEntity implements Angerable {
    ...
    public LampGolemEntity(EntityType<? extends TameableEntity> entityType, World world) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if ((item == Items.GLOWSTONE || item == Items.SHROOMLIGHT ) && !isTamed()) {
        ...
        if((itemStack.getItem() ==Items.GLOWSTONE) || (itemStack.getItem() == Items.SHROOMLIGHT)){
    ...
    public void tickMovement() {
        ...
        double x = this.getX() + (random.nextDouble() - 0.5D) * this.getWidth();
        double y = this.getY() + random.nextDouble() * this.getHeight();
        double z = this.getZ() + (random.nextDouble() - 0.5D) * this.getWidth();

        world.addParticle(ParticleTypes.FLAME, x,y,z,0.0f,0.0f,0.0f);

        BlockPos lightPos = new BlockPos(this.getPos());
        if (!lightPos.equals(this.lastLightPos)) {
            this.world.setBlockState(lightPos, Blocks.LIGHT.getDefaultState().with(Properties.LEVEL_15, 15));

            // Entferne den Light Block von der vorherigen Position
            if (this.lastLightPos != null) {
                this.world.removeBlock(this.lastLightPos, false);
            }
